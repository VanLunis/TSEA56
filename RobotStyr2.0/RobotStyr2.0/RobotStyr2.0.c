...
void turn_left()
{
	...
	else if(!fwall && !rwall && distance_back > WALLS_MAX_DISTANCE)
	{
...
void turn_right()
{
	...
	else if(!fwall && !lwall && distance_back > WALLS_MAX_DISTANCE)
	{
		turn_right_control_on_zero_walls();
	}
	else if(!fwall && lwall && distance_back > WALLS_MAX_DISTANCE)
...
void run_direction_command(unsigned char direction_command)
{
	...
	in_turn = 0;
	driven_distance = 0;
	if(!no_forward && !(fwall && lwall && rwall))
	{
		turn_forward();
	}
	add_to_buffer(&send_buffer, 0x70, (char) un+1);
...
void mission_phase_5() // Go forward short, leave object, back out
{
	driven_distance = 0;
	add_to_buffer(&send_buffer, 0x10, (char) missionPhase);
	...
	else if(command[c-1] == 'r')
	{
		turn_right();
	}
	else if(command[c-1] == 'l')
	{
		turn_left();
	}
	else if(command[c-1] == 'f')
	{
		turn_back();
	}
	in_turn = 0;
