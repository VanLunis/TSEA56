...
void init_master(void)
{
	...
	PORTB = (1<<PORTB4)|(1<<PORTB3); // Pulling SS2 and SS1 high
...
void send(struct data_buffer* my_buffer, int slave)
{
	...
	if (slave == 1)
	{
		...
		PORTB = (0<<PORTB0); // Order slave 1 to adapt receive mode
		PORTB = (1<<PORTB0);
	}
	else if(slave == 2)
	{
	...
	while(1)
	{
		if (slave == 1)
		{
			PORTB = (0<<PORTB3); // Pulling SS1 low
		}
		else if(slave==2)
		{
		...
		else
		{
			...
			if(local_failed_attempts > 10000) // Ten thousand tries allowed before leaving function
			{
				transmission_status = 0;
				break;
			}
...
void receive(int slave)
{
	...
	{
		current_slave_ready = &slave1_ready;
		PORTD = (0<<PORTD6); // Order slave 1 to adapt send mode
		PORTD = (1<<PORTD6);
	}
	else if(slave == 2)
	{
...
int main(void)
{
	...
	for(int i=1; i<64; i++)
	{
		add_to_buffer(&slave1_buffer,i,i);
	}
	
	while(buffer_empty(&slave1_buffer) == 0)
	{	
		send_to_slave1();
	}
